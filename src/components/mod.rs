...
pub(crate) const fn build_set_flag_bit<C: Component, G: CuttleGroup, T, const SET: bool>(
    pos: u8,
) -> impl FnMut(Trigger<T, C>, ResMut<IndexArena<C>>, Query<&mut CuttleFlags, Or<(With<G>, With<Extension<G>>)>>) {
...
mod tests {
    ...
    fn set_flags_multiple_groups() {
        ...
        let ent4 = world.spawn((Sdf, CuttleFlags::default(), Comp::<0>, Comp::<1>, Comp::<2>, Comp::<3>)).id();
        let ent5 = world.spawn((UiSdf, CuttleFlags::default(), Comp::<0>, Comp::<1>, Comp::<2>, Comp::<3>)).id();
        ...
        let ent1 = world.spawn((CuttleFlags::default(), Comp::<0>)).id();
        let ent2 = world.spawn((Extension::<Sdf>::new(ent2), CuttleFlags::default(), Comp::<0>)).id();
        let ent3 = world.spawn((Extension::<UiSdf>::new(ent3), CuttleFlags::default(), Comp::<0>)).id();
        let ent4 = world.spawn((Extension::<Sdf>::new(ent4), CuttleFlags::default(), Comp::<0>, Comp::<1>, Comp::<2>, Comp::<3>)).id();
        let ent5 = world.spawn((Extension::<UiSdf>::new(ent5), CuttleFlags::default(), Comp::<0>, Comp::<1>, Comp::<2>, Comp::<3>)).id();
